...
% Title Information
\title{\textbf{Prototype of Solar Based \\ Waste Water Reclaiming System}}
...
\begin{titlepage}
  ...
  \includegraphics[width=0.36\textwidth]{assets/NITP_LOGO.png}\par
  \vspace{0.6cm}
    
    {\Huge \textbf{Prototype of Solar Based \\[0.5em] Waste Water Reclaiming System} \par}
    \vspace{1.0cm}
...
\begin{enumerate}
    \item \textbf{PI-based control for Fresnel solar collectors in wastewater reclamation} \citep{rodriguez2024}: This study presents a demonstration-scale integration of a 36-m$^2$ Fresnel solar collector used for water disinfection. The authors implemented a PI-based control approach for the automatic operation of the photoreactor, successfully achieving EU water quality standards while maintaining operational temperatures up to 60$^\circ$C.
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.8\textwidth]{assets/ControlSystemDiagram.jpeg}
    \caption{Closed-Loop Proportional-Based Control System Block Diagram}
    \label{fig:control_diagram}
\end{figure}

As shown in the block diagram (Figure \ref{fig:control_diagram}), a closed-loop proportional-based control logic is implemented to mitigate this. For this minor project prototype, a robust Proportional control approach was emphasized, acting as the primary responsive element in the overall feedback loop. A base motor speed ($PWM_{base}$) is defined to maintain a minimum continuous fluid flow, ensuring the sensors always read active fluid temperatures rather than stagnant pockets. 
...
This control relationship allows the primary variable (temperature) to directly influence the actuator command (motor PWM), which in turn changes the flow rate, improving disturbance rejection and maintaining process stability.
...
\bibitem[Rodríguez-García et al., 2024]{rodriguez2024} Rodríguez-García, D., García Sánchez, J. L., Guzmán, J. L., Casas López, J. L., \& Sánchez Pérez, J. A. (2024). PI-based control for Fresnel solar collectors in wastewater reclamation. \textit{4th IFAC Conference on Advances in Proportional-Integral-Derivative Control}.
...
\section*{Arduino Source Code}
The following is the complete C++ source code flashed to the Arduino microcontroller to handle sensor data acquisition and proportional-based control.
...
void loop() {
  ...
  if (deltaTemp < 0) deltaTemp = 0;
    ...
    // NOTE: Speed increases with temp difference
  float control = Kp * deltaTemp;
