  ...
  if (!history || history.length === 0) return (
    <div className="glass-card p-5">
      ...
      <div className="flex flex-col items-center justify-center py-10 text-center">
        <div className="h-10 w-10 rounded-full bg-theme-hover flex items-center justify-center mb-3">
        ...
        <table className="w-full text-xs">
          <thead>
            <tr className="border-b border-theme-subtle">
              {COLUMNS.map((col) => (
                <th key={col.key} className={`pb-2.5 font-medium text-text-muted uppercase tracking-wider px-2 first:pl-0 last:pr-0 ${col.align === 'right' ? 'text-right' : 'text-left'}`}>
                  {col.label}
                </th>
              ))}
            </tr>
          ...
          <tbody className="divide-y divide-theme-subtle">
            {reversed.slice(0, 20).map((row, i) => (
              <tr key={i} className="hover:bg-theme-hover transition-colors duration-100">
                {COLUMNS.map((col) => (
                  <td key={col.key} className={`py-2 px-2 first:pl-0 last:pr-0 ${col.align === 'right' ? 'text-right' : 'text-left'}`}>
                    {col.key === 'deltaTemp' ? (
