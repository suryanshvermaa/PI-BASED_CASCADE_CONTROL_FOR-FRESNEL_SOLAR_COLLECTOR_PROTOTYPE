...
import { toneToClasses } from '../../utils/formatters';

/**
 * Animated metric card with ease-out number interpolation.
 * Props:
 *   title    — label text
 *   value    — numeric sensor value (null = loading/unavailable)
 *   unit     — unit string e.g. "°C", "L/min"
 *   decimals — decimal places (0 for integers like PWM)
...
export default function StatsCard({
  ...
  // Animate numeric transitions (ease-out cubic)
  useEffect(() => {
    if (rafRef.current) {
  ...
  return (
    <div
      className={`
        ...
        ${bg} ${border}
        hover:-translate-y-0.5 hover:shadow-lg hover:border-white/[0.12]
        ${flash ? 'scale-[1.02]' : ''}
    ...
    >
      ...
      <div className="flex items-baseline gap-1.5 min-w-0">
        ...
        {unit && (
          <span className="text-sm text-text-muted flex-shrink-0">{unit}</span>
        )}
      </div>
