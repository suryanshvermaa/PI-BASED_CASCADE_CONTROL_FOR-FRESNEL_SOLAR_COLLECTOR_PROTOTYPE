...
export default function DeviceStatus({ mode, socketConnected, isLiveMode, overallConnected, lastReceivedAt }) {
  const statusLabel = !socketConnected ? 'Disconnected'
    : isLiveMode ? (overallConnected ? 'Arduino Online' : 'Arduino Offline') : 'Mock Mode Active';

  const statusColor = !socketConnected ? 'text-danger'
    : isLiveMode ? (overallConnected ? 'text-success' : 'text-danger') : 'text-warning';

  const dotColor = !socketConnected ? 'bg-danger'
    : isLiveMode ? (overallConnected ? 'bg-success animate-pulse-dot' : 'bg-danger') : 'bg-warning animate-pulse-dot';
      ...
      <div className="space-y-3">
        ...
        <div className="flex items-center justify-between">
          ...
          <span className={`text-xs font-medium px-2 py-0.5 rounded-full border
            ${isLiveMode ? 'bg-success/10 border-success/25 text-success' : 'bg-warning/10 border-warning/25 text-warning'}
          `}>
        ...
        <div className="flex items-center justify-between pt-2 border-t border-theme-subtle">
          ...
          <span className="text-xs text-text-secondary">{formatRelativeTime(lastReceivedAt)}</span>
        </div>
      </div>
    </div>
  );
